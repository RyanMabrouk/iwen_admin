    ...
    <FormControl className="group flex w-fit flex-col gap-3">
      ...
      <div className="relative flex flex-row items-center justify-center gap-1.5 ">
        <Select
          ...
          className={cn(
            `group peer border transition-all ease-linear first-letter:capitalize max-[515px]:w-[7.5rem] [&_.Mui-selected]:!bg-red-500 [&_.MuiOutlinedInput-notchedOutline]:border-none ${open ? "rounded-b-none rounded-t-sm shadow-md" : "rounded-sm shadow-sm hover:shadow-md"} ${
              variant === "regular"
                ? "h-9 w-[12.5rem]"
                ...
                (option: SelectGenericOption, i: number): JSX.Element => {
                  return option?.group_name ? (
                    ...
                    <MenuItem
                     dir="rtl"
                      value={option?.value}
                      className={`peer !max-h-10 !px-2 !py-2 text-[0.95rem] capitalize transition-all ease-linear hover:!bg-red-500 hover:text-white ${
            ...
            : options?.map((option, i) => {
                return (
                  <MenuItem
                    dir="rtl"
                    value={option?.value}
                    className={`peer transition-all ease-linear first-letter:capitalize hover:!bg-red-500 hover:text-white ${capitalize ? "capitalize" : ""} `}
